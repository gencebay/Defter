 ...
 * @param {string} [propsInterfaceOrEnumName] Name of an interface or enum
 * if you only want to parse said interface or enum.
 * @returns {Array<IProperty>} An array of properties.
 */
export function parse(
  source: string,
  propsInterfaceOrEnumName?: string
): IProperty[] {
  let props: IProperty[] = [];
  let regex: RegExp | null = null;
  let parseInfo;

  let propertyNameSuffix = (type: string) =>
    type === "interface" ? " interface" : " enum";
  let propertyType = (type: string) =>
    type === "interface" ? PropertyType.interface : PropertyType.enum;

  if (propsInterfaceOrEnumName) {
    regex = new RegExp(
      `export (interface|enum) ${propsInterfaceOrEnumName}(?: extends .*?)? \\{(.*[\\r\\n]*)*?\\}`
    );
    ...
    if (regexResult && regexResult.length > 0) {
      ...
      return [
        <IProperty>{
          ...
          propertyName:
            propsInterfaceOrEnumName + propertyNameSuffix(regexResult[1]),
    ...
    regex = new RegExp(
      `export (interface|enum) (\\S*?)(?: extends .*?)? \\{(.*[\\r\\n]*)*?\\}`,
      "g"
    );
...
function _parseEnumOrInterface(
